...
今天不会提交或改动教材仓库。所有会写入 Git 历史的练习都在 `learner_outputs/day04/sandbox_repo/` 临时练习仓库进行；教材脚本本身只读取当前仓库状态。
...
```bash
sed -n '1,120p' day04/code/minimal_git_status.py
python3 day04/code/minimal_git_status.py
```
...
```bash
python3 day04/code/git_evidence.py --help
python3 day04/code/git_evidence.py
echo $?
sed -n '1,120p' learner_outputs/day04/git_evidence.json
```
...
```bash
mkdir -p learner_outputs/day04/sandbox_repo
cd learner_outputs/day04/sandbox_repo
git init
...
```bash
cd "$(git rev-parse --show-toplevel)"
python3 -m unittest -v day04.tests.test_git_evidence
```
...
```bash
mkdir -p learner_outputs/day04/not_a_repo
python3 day04/code/git_evidence.py --repo learner_outputs/day04/not_a_repo
echo $?
```
