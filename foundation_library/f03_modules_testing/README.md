...
退出码应为 `0`。今天会继续读取 `foundation_library/f02_csv_json/data/mini_episodes.csv`，并新增：

```text
foundation_library/f03_modules_testing/
├── README.md
├── code/
│   ├── __init__.py
│   ├── minimal_schema.py
...
运行后还会产生 `learner_outputs/foundation_library/f03_modules_testing/validated_manifest.json`。它是个人练习输出，已被 Git 忽略。
...
相对路径 `foundation_library/f02_csv_json/data/mini_episodes.csv` 从 shell 的当前目录解释。如果在仓库外运行，路径会失效。`__file__` 则指当前模块文件；本课使用：

```python
repo_root = Path(__file__).resolve().parents[3]
```

路径层级是 `仓库/foundation_library/f03_modules_testing/code/episode_schema.py`：`parents[0]` 是 `code`，`parents[1]` 是 `f03_modules_testing`，`parents[2]` 是 `foundation_library`，`parents[3]` 才是仓库根。然后用 `/` 运算符拼路径。这里的 `/` 不是除法，而是 `Path` 提供的路径组合语法。

这种写法解决“从哪里启动”的问题，但不是宇宙通用方案：如果随意移动文件，固定的 `parents[3]` 仍会错。因此仓库结构也是程序契约的一部分。未来安装成 package 后会学习更正式的资源定位。
...
```bash
sed -n '1,200p' foundation_library/f03_modules_testing/code/minimal_schema.py
python3 foundation_library/f03_modules_testing/code/minimal_schema.py
```
...
```bash
python3 -m py_compile foundation_library/f03_modules_testing/code/minimal_schema.py
```
...
```bash
sed -n '1,100p' foundation_library/f03_modules_testing/code/episode_schema.py
sed -n '101,220p' foundation_library/f03_modules_testing/code/episode_schema.py
```
...
```bash
python3 foundation_library/f03_modules_testing/code/episode_schema.py --help
```

再运行默认输入：

```bash
python3 foundation_library/f03_modules_testing/code/episode_schema.py
echo $?
```
...
Levels: L0, L1, L2
Saved: .../learner_outputs/foundation_library/f03_modules_testing/validated_manifest.json
0
```
...
```bash
sed -n '1,100p' learner_outputs/foundation_library/f03_modules_testing/validated_manifest.json
```
...
```bash
mkdir -p learner_outputs/foundation_library/f03_modules_testing
cp foundation_library/f02_csv_json/data/mini_episodes.csv learner_outputs/foundation_library/f03_modules_testing/my_episodes.csv
```
...
```bash
python3 foundation_library/f03_modules_testing/code/episode_schema.py \
  --input learner_outputs/foundation_library/f03_modules_testing/my_episodes.csv \
  --output learner_outputs/foundation_library/f03_modules_testing/my_manifest.json
```
...
```bash
cp foundation_library/f03_modules_testing/tests/test_episode_schema.py \
  learner_outputs/foundation_library/f03_modules_testing/my_test_episode_schema.py
```

把 `self.assertEqual(manifest["episode_count"], 1)` 中的 `1` 改成 `2`。预测失败信息会显示哪个 expected/actual，再运行：

```bash
python3 -m unittest -v learner_outputs/foundation_library/f03_modules_testing/my_test_episode_schema.py
```
...
| 现象 | 先检查 | 常见原因 | 止损时间 |
|---|---|---|---:|
| `ModuleNotFoundError: foundation_library` | `pwd` 与仓库根 | 在 `foundation_library/f03_modules_testing/tests` 内启动测试 | 10 分钟 |
| `FileNotFoundError` | `--input` 实际路径 | 相对路径从当前目录解释 | 15 分钟 |
| `IndexError` 出现在 `parents[3]` | 脚本是否被移动 | 复制到了不同目录层级 | 15 分钟 |
