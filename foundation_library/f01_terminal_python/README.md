...
```text
foundation_library/f01_terminal_python/code/first_run.py
```
...
├── README.md
├── foundation_library/f01_terminal_python/
│   ├── README.md
│   └── code/
│       └── first_run.py
└── learner_outputs/
...
```bash
sed -n '1,240p' foundation_library/f01_terminal_python/code/first_run.py
```
...
```bash
python3 foundation_library/f01_terminal_python/code/first_run.py --help
```
...
```bash
python3 foundation_library/f01_terminal_python/code/first_run.py
```
...
Saved: .../learner_outputs/foundation_library/f01_terminal_python/first_run.txt
```
...
```bash
sed -n '1,80p' learner_outputs/foundation_library/f01_terminal_python/first_run.txt
```
...
```bash
cp foundation_library/f01_terminal_python/code/first_run.py \
  learner_outputs/foundation_library/f01_terminal_python/my_first_run.py
```
...
```bash
python3 learner_outputs/foundation_library/f01_terminal_python/my_first_run.py
```

注意：复制后的 `source_script=` 会指向你的个人副本；因为本课指定的复制位置仍然恰好位于教程根目录下三级，`Saved:` 仍落在 `learner_outputs/foundation_library/f01_terminal_python/`。这说明 `__file__` 记录“当前文件在哪里”，而路径计算还取决于目录层级。把现象写入 `day01_notes.md`，不要急着重构；Day 3 之后再系统学习模块与路径设计。

### 实验 C：故意给 seed 一个错误类型

```bash
python3 foundation_library/f01_terminal_python/code/first_run.py --seed seven
echo $?
```
...
在 `learner_outputs/foundation_library/f01_terminal_python/` 中保存：
