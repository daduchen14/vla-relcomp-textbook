...
.venv-foundation_library/f06_environments_dependencies/bin/python -c 'import torch; print(torch.__version__, torch.__file__)'
```

若还没有 torch，按照官方“Start Locally”页面为自己的 OS 选择 Pip/Python/CPU 命令。macOS 当前可在课程虚拟环境执行：

```bash
.venv-foundation_library/f06_environments_dependencies/bin/python -m pip install torch
```

安装是免费的但会下载较大 wheel；先用 Day 5 快照确认空间。不要使用 `sudo pip`，不要为 CUDA 随意安装非本机 wheel。教程作者本次下载 torch wheel 约 111 MB，实际大小随版本/平台变化。

今天新增最小 tensor 脚本、完整 `tensor_lab.py`、4 项 CPU 测试和个人 JSON 报告。运行命令统一使用 `.venv-foundation_library/f06_environments_dependencies/bin/python`，避免系统 Python 找不到 torch。
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python -m pip install -r foundation_library/f09_tensors/config/requirements-cpu.txt
```
...
```bash
sed -n '1,160p' foundation_library/f09_tensors/code/minimal_tensors.py
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f09_tensors/code/minimal_tensors.py
```
...
```bash
sed -n '1,140p' foundation_library/f09_tensors/code/tensor_lab.py
sed -n '141,300p' foundation_library/f09_tensors/code/tensor_lab.py
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f09_tensors/code/tensor_lab.py --help
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f09_tensors/code/tensor_lab.py --device cpu --batch-size 2
echo $?
```
...
```bash
sed -n '1,240p' learner_outputs/foundation_library/f09_tensors/tensor_report.json
```
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f09_tensors/code/tensor_lab.py --device cuda
echo $?
```
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python -m unittest -v foundation_library.f09_tensors.tests.test_tensor_lab
.venv-foundation_library/f06_environments_dependencies/bin/python -m py_compile \
  foundation_library/f09_tensors/code/minimal_tensors.py \
  foundation_library/f09_tensors/code/tensor_lab.py \
  foundation_library/f09_tensors/tests/test_tensor_lab.py
```
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f09_tensors/code/tensor_lab.py --batch-size 5
```
...
| 现象 | 先检查与处理 | 止损时间 |
|---|---|---:|
| system Python 找不到 torch | 使用 `.venv-foundation_library/f06_environments_dependencies/bin/python` 并打印 executable | 15 分钟 |
