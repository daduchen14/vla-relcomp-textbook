...
- `foundation_library/f15_cnn/code/minimal_convolution.py`：固定卷积核的最小可运行例子；
- `foundation_library/f15_cnn/code/cnn_lab.py`：fixture 图像数据集、CNN、训练、评测和 JSON 证据；
- `foundation_library/f15_cnn/tests/test_cnn_lab.py`：数据、shape、错误输入、学习能力与复现测试；
- `learner_outputs/foundation_library/f15_cnn/cnn_report.json`：本机运行后生成的个人练习结果，不提交 Git。
...
.venv-foundation_library/f06_environments_dependencies/bin/python --version
.venv-foundation_library/f06_environments_dependencies/bin/python -c "import torch; print(torch.__version__)"
```

预期看到教材分支、Python 版本和 PyTorch 版本。如果 `.venv-foundation_library/f06_environments_dependencies/bin/python` 不存在，按 Day 6 建环境，再按 Day 9 的固定依赖安装；止损时间 10 分钟，不要改用一个来源不明的全局环境凑合。
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f15_cnn/code/minimal_convolution.py
```
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f15_cnn/code/cnn_lab.py
```
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python -m json.tool learner_outputs/foundation_library/f15_cnn/cnn_report.json | sed -n '1,60p'
```
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python -m unittest foundation_library.f15_cnn.tests.test_cnn_lab -v
```
...
修改 `foundation_library/f15_cnn/code/minimal_convolution.py`：
...
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f15_cnn/code/cnn_lab.py \
  --channels 1 \
  --output learner_outputs/foundation_library/f15_cnn/channels_1.json
```
...
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f15_cnn/code/cnn_lab.py \
  --epochs 1 \
  --output learner_outputs/foundation_library/f15_cnn/epoch_1.json
```
...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f15_cnn/code/cnn_lab.py --seed 22 \
  --output learner_outputs/foundation_library/f15_cnn/seed22_a.json
.venv-foundation_library/f06_environments_dependencies/bin/python foundation_library/f15_cnn/code/cnn_lab.py --seed 22 \
  --output learner_outputs/foundation_library/f15_cnn/seed22_b.json
diff -u learner_outputs/foundation_library/f15_cnn/seed22_a.json learner_outputs/foundation_library/f15_cnn/seed22_b.json
```
