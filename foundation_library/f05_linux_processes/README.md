...
```text
foundation_library/f05_linux_processes/code/minimal_process.py
foundation_library/f05_linux_processes/code/system_snapshot.py
foundation_library/f05_linux_processes/tests/test_system_snapshot.py
learner_outputs/foundation_library/f05_linux_processes/system_snapshot.json   # 运行后生成，不提交
```
...
macOS/Linux 路径从根目录 `/` 开始。`/Users/...` 或 `/home/...` 是绝对路径；`foundation_library/f05_linux_processes/code` 是从当前工作目录解释的相对路径。`.` 代表当前目录，`..` 代表父目录。目录不是“装文件的图标”，而是一组名字到对象的映射。

常用只读命令：

```bash
pwd                         # 当前工作目录
ls -la foundation_library/f05_linux_processes                # 包含隐藏项的长格式列表
find foundation_library/f05_linux_processes -maxdepth 3 -type f | sort
file foundation_library/f05_linux_processes/code/system_snapshot.py
```
...
```bash
python3 foundation_library/f05_linux_processes/code/minimal_process.py \
  > learner_outputs/foundation_library/f05_linux_processes/stdout.txt \
  2> learner_outputs/foundation_library/f05_linux_processes/stderr.txt
echo $?
```
...
```bash
mkdir -p learner_outputs/foundation_library/f05_linux_processes
sed -n '1,160p' foundation_library/f05_linux_processes/code/minimal_process.py
python3 foundation_library/f05_linux_processes/code/minimal_process.py
echo $?
```
...
```bash
sed -n '1,110p' foundation_library/f05_linux_processes/code/system_snapshot.py
sed -n '111,240p' foundation_library/f05_linux_processes/code/system_snapshot.py
```
...
```bash
python3 foundation_library/f05_linux_processes/code/system_snapshot.py --help
python3 foundation_library/f05_linux_processes/code/system_snapshot.py --probe-exit-code 0
echo $?
sed -n '1,160p' learner_outputs/foundation_library/f05_linux_processes/system_snapshot.json
```
...
```bash
python3 foundation_library/f05_linux_processes/code/system_snapshot.py --probe-exit-code 3
echo $?
```
...
```bash
python3 -m unittest -v foundation_library.f05_linux_processes.tests.test_system_snapshot
...
复制最小脚本到 `learner_outputs/foundation_library/f05_linux_processes/my_process.py`，把子进程 exit code 改成 2。先写下父脚本 stdout、stderr 和 shell `$?` 的预测，再分别重定向运行。查看两个文件，解释为什么子进程 stderr 最终由父程序作为普通文字打印到了父 stdout。
...
```bash
FIXTURE_MODE=careful python3 foundation_library/f05_linux_processes/code/system_snapshot.py
rg 'FIXTURE_MODE' learner_outputs/foundation_library/f05_linux_processes/system_snapshot.json || true
```
...
```bash
cd foundation_library/f05_linux_processes/code
python3 system_snapshot.py --probe-exit-code 1
cd "$(git rev-parse --show-toplevel)"
```

预测仓库根和默认输出是否仍正确。预期脚本用 Git 和 `__file__` 定位，仍写入根下 `learner_outputs/foundation_library/f05_linux_processes`；JSON 的 current directory 则诚实记录 `foundation_library/f05_linux_processes/code`。
...
```bash
python3 -c 'import json; from pathlib import Path; d=json.loads(Path("learner_outputs/foundation_library/f05_linux_processes/system_snapshot.json").read_text()); print(round(d["free_bytes"] / 1024**3, 2), "GiB")'
```
