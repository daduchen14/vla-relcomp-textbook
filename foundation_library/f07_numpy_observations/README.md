...
```bash
.venv-foundation_library/f06_environments_dependencies/bin/python -m pip install 'numpy>=2.0,<3'
.venv-foundation_library/f06_environments_dependencies/bin/python -c 'import numpy as np; print(np.__version__)'
```

这会访问 PyPI，但不需要付费。网络失败时最长排查 20 分钟；可先阅读正文，不要使用 `sudo pip` 或随机镜像。后续命令中的 `python3` 应替换成成功导入 NumPy 的准确解释器。

今天代码位于 `foundation_library/f07_numpy_observations/code/`，个人输出是 `learner_outputs/foundation_library/f07_numpy_observations/fixture_arrays.npz` 和 `array_summary.json`。
...
```bash
sed -n '1,160p' foundation_library/f07_numpy_observations/code/minimal_arrays.py
python3 foundation_library/f07_numpy_observations/code/minimal_arrays.py
```
...
```bash
sed -n '1,130p' foundation_library/f07_numpy_observations/code/sensor_pipeline.py
sed -n '131,280p' foundation_library/f07_numpy_observations/code/sensor_pipeline.py
python3 foundation_library/f07_numpy_observations/code/sensor_pipeline.py --help
...
Saved arrays: .../learner_outputs/foundation_library/f07_numpy_observations/fixture_arrays.npz
Saved summary: .../learner_outputs/foundation_library/f07_numpy_observations/array_summary.json
...
```bash
sed -n '1,160p' learner_outputs/foundation_library/f07_numpy_observations/array_summary.json
python3 -c 'import numpy as np; d=np.load("learner_outputs/foundation_library/f07_numpy_observations/fixture_arrays.npz"); print(d.files); print(d["image"].shape, d["action"])'
```
...
python3 -m py_compile \
  foundation_library/f07_numpy_observations/code/minimal_arrays.py \
  foundation_library/f07_numpy_observations/code/sensor_pipeline.py \
  foundation_library/f07_numpy_observations/tests/test_sensor_pipeline.py
```
...
```bash
python3 foundation_library/f07_numpy_observations/code/sensor_pipeline.py --height 3 --width 5
```
...
```bash
python3 foundation_library/f07_numpy_observations/code/sensor_pipeline.py --action-scale 2
sed -n '1,160p' learner_outputs/foundation_library/f07_numpy_observations/array_summary.json
```
