...
python3 day05/code/system_snapshot.py
command -v python3
...
```text
day06/config/fixture_requirements.json
day06/code/minimal_import_check.py
day06/code/environment_doctor.py
day06/tests/test_environment_doctor.py
```

个人虚拟环境统一放在 `.venv-day06/`，根 `.gitignore` 已忽略 `.venv*`。运行报告写入 `learner_outputs/day06/`，都不提交。
...
source .venv-day06/bin/activate
python -c 'import sys; print(sys.executable); print(sys.prefix != sys.base_prefix)'
python -m pip --version
```

Windows PowerShell 的激活命令不同；本项目后期面向 Linux，教程统一使用 POSIX shell。激活主要修改当前 shell 的 PATH；关闭终端后需重新激活。更稳妥的自动化命令可以直接使用 `.venv-day06/bin/python`，不依赖激活状态。
...
```bash
sed -n '1,120p' day06/code/minimal_import_check.py
python3 day06/code/minimal_import_check.py
echo $?
```
...
```bash
sed -n '1,160p' day06/config/fixture_requirements.json
sed -n '1,120p' day06/code/environment_doctor.py
sed -n '121,260p' day06/code/environment_doctor.py
```
...
```bash
python3 day06/code/environment_doctor.py --help
python3 day06/code/environment_doctor.py
echo $?
sed -n '1,200p' learner_outputs/day06/environment_report.json
```
...
```bash
python3 -m venv .venv-day06
.venv-day06/bin/python day06/code/environment_doctor.py \
  --output learner_outputs/day06/venv_report.json
.venv-day06/bin/python -m pip --version
```

预期 venv 报告中 `inside_virtual_environment=true`，Python executable 位于 `.venv-day06/bin/`。清单仍 ready，因为只要求标准库。实际 Python 小版本取决于创建 venv 的基础解释器。

## 6. 自动化测试

```bash
python3 -m unittest -v day06.tests.test_environment_doctor
...
运行系统 Python 与 `.venv-day06/bin/python` 两份报告。先预测哪些字段相同、哪些不同，再比较：

```bash
python3 -c 'import json; from pathlib import Path; a=json.loads(Path("learner_outputs/day06/environment_report.json").read_text()); b=json.loads(Path("learner_outputs/day06/venv_report.json").read_text()); print("system", a["python_executable"], a["inside_virtual_environment"]); print("venv", b["python_executable"], b["inside_virtual_environment"])'
```
...
```bash
cp day06/config/fixture_requirements.json learner_outputs/day06/my_requirements.json
python3 day06/code/environment_doctor.py \
  --manifest learner_outputs/day06/my_requirements.json \
  --output learner_outputs/day06/missing_required_report.json
echo $?
```
...
```bash
.venv-day06/bin/python -m pip list
.venv-day06/bin/python -m pip freeze \
  > learner_outputs/day06/venv_freeze.txt
