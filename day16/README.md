...
- `day16/code/minimal_embedding.py`：24 行左右的字符 token、查表和可学习位置示例；
- `day16/code/sequence_lab.py`：确定性字符词表、批量补齐、mask、正弦位置编码和 JSON 报告；
- `day16/tests/test_sequence_lab.py`：6 个数据契约与复现测试；
- `learner_outputs/day16/sequence_report.json`：本机生成的教学输出，不提交 Git。
...
```bash
.venv-day06/bin/python day16/code/minimal_embedding.py
```
...
```bash
.venv-day06/bin/python day16/code/sequence_lab.py
```
...
.venv-day06/bin/python -m json.tool \
  learner_outputs/day16/sequence_report.json | sed -n '1,100p'
```
...
```bash
.venv-day06/bin/python -m unittest day16.tests.test_sequence_lab -v
.venv-day06/bin/python day16/code/sequence_lab.py --help
```
...
.venv-day06/bin/python day16/code/sequence_lab.py \
  --embedding-dim 16 \
  --output learner_outputs/day16/dim16.json
```
...
```bash
.venv-day06/bin/python day16/code/sequence_lab.py --embedding-dim 7
echo $?
```
...
```bash
.venv-day06/bin/python day16/code/sequence_lab.py --max-length 3
echo $?
```
