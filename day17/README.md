...
- `day17/code/minimal_attention.py`：约 28 行的矩阵乘法注意力；
- `day17/code/attention_lab.py`：带投影、缩放、padding mask、错误检查和 JSON 证据的单头 self-attention；
- `day17/tests/test_attention_lab.py`：shape、归一化、mask、梯度、非法输入和复现测试；
- `learner_outputs/day17/attention_report.json`：本机生成的 fixture 权重记录，不提交 Git。
...
```bash
.venv-day06/bin/python day17/code/minimal_attention.py
```
...
```bash
.venv-day06/bin/python day17/code/attention_lab.py
```
...
.venv-day06/bin/python -m json.tool \
  learner_outputs/day17/attention_report.json | sed -n '1,140p'
```
...
```bash
.venv-day06/bin/python -m unittest day17.tests.test_attention_lab -v
.venv-day06/bin/python day17/code/attention_lab.py --help
```
...
```bash
.venv-day06/bin/python day17/code/attention_lab.py --temperature 0.25 \
  --output learner_outputs/day17/temp025.json
.venv-day06/bin/python day17/code/attention_lab.py --temperature 4.0 \
  --output learner_outputs/day17/temp4.json
```
...
```bash
.venv-day06/bin/python -m unittest \
  day17.tests.test_attention_lab.AttentionLabTests.test_padding_key_and_query_are_zeroed -v
```
...
```bash
.venv-day06/bin/python - <<'PY'
import torch
from day17.code.attention_lab import SingleHeadSelfAttention
