#!/usr/bin/env python3
"""验收 A/B 语言 oracle 重建与挑战解释。"""
from __future__ import annotations
import argparse,csv,json
from pathlib import Path
try:from .analyze_language_oracle import analyze
except ImportError:from analyze_language_oracle import analyze
def read(path):
    with path.open(encoding="utf-8",newline="") as handle:return list(csv.DictReader(handle))
def check(source,output,report):
    rows,expected=analyze(source)
    if read(output)!=rows or json.loads(report.read_text(encoding="utf-8"))!=expected:raise ValueError("oracle 产物不可重建")
...
def main():
    ...
    memo=a.challenge_memo.read_text(encoding="utf-8").strip();required=("normalized instruction","TARGET","START","ACTION","GOAL","BDDL truth","recovery","damage","stage effect","paired","leakage","synthetic","causal")
    if len(memo)<240 or not all(token in memo for token in required):raise ValueError("challenge memo 不完整")
    print("PASS: Day 33 language-normalization oracle analysis")
